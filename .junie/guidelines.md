...
- Do: バージョンを上げる場合は client/Cargo.toml の version と migration の両方を更新する
- Do: migration のテストは最低3パターン（マイグレーション実行、既存データ保持、スキップ）を実装する
- Do: iced の KeyPressed/KeyReleased イベントでは key と modified_key の両方を考慮する。US キーボードで Shift+= → "+" のように修飾キー適用後の文字が異なる場合がある。
...
- client/src/feature/migrate/ バージョンごとのマイグレーション関数。migrate_X_Y_Z.rs の命名規則に従う。
